...
src_prepare() {
	local libressl_patch="${FILESDIR}/1.70.0-libressl-openssl-sys.patch"
	pushd vendor/openssl-sys > /dev/null || die
	eapply "${libressl_patch}"
	"${EPREFIX}"/bin/sh "${FILESDIR}"/rehash-crate.sh "${libressl_patch}" || die
	popd > /dev/null || die
